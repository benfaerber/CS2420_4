 ...
 * @version 2025-05-22
 */
public abstract class SpeedyTimingExperiment {
    ...
    public void printResults() {
        ...
        int batchSize = 1000;
        int batchCount = (int) Math.ceil((double) problemSizeCount / batchSize);

        ExecutorService executor = Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors());
        Future<StringBuilder>[] futures = new Future[batchCount];

        for (int b = 0; b < batchCount; b++) {
            final int batchStartIndex = b * batchSize;
            final int batchEndIndex = Math.min(batchStartIndex + batchSize, problemSizeCount);

            futures[b] = executor.submit(() -> {
                StringBuilder batchResults = new StringBuilder();
                int size = problemSizeMin + batchStartIndex * problemSizeStep;

                for (int i = batchStartIndex; i < batchEndIndex; i++) {
                    long medianElapsed = computeMedianElapsedTime(size);
                    synchronized (System.out) {
                        System.out.println(size + "\t" + medianElapsed);
                    }
                    batchResults.append(medianElapsed).append("\n");
        ...
        for (Future<StringBuilder> future : futures) {
            try {
                finalResults.append(future.get());
            } catch (InterruptedException | ExecutionException e) {
                e.printStackTrace();
            }
        }

        ClipboardHelper.copyToClipboard(finalResults.toString());
    }
